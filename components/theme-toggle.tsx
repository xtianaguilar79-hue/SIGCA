...
export function ThemeToggle(){
  ...
  useEffect(()=>{const timer=window.setTimeout(()=>{const saved=localStorage.getItem("sigca-theme") as Theme|null;const initial=saved||(window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");setTheme(initial);document.documentElement.dataset.theme=initial},0);return()=>window.clearTimeout(timer)},[]);
