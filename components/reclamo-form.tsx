...
export function ReclamoForm({ users, currentCompany }: { users: User[]; currentCompany?: string | null }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      empresa_id: null, empresa_nombre: audience === "empresa" ? (currentCompany?.trim() || null) : null, lugar: null, fecha_hecho: optional("fecha_hecho"),
      categoria: optional("categoria"), descripcion: String(data.get("descripcion") || "").trim(),
      prioridad: "normal", estado: "abierto",
  ...
  return <form className="claim-form" onSubmit={save}>
    <div className="claim-grid"><label>Título<input name="titulo" required maxLength={180}/></label><label>Fecha del hecho<input name="fecha_hecho" type="date"/></label></div>
    <label>Categoría<select name="categoria" defaultValue="laboral"><option value="laboral">Laboral</option><option value="salarial">Salarial</option><option value="higiene_seguridad">Higiene y seguridad</option><option value="obra_social">Obra social</option><option value="afiliacion">Afiliación</option><option value="otro">Otro</option></select></label>
