...
export type ActaMinutaInitialData = {
  tipo: string; titulo: string; fecha: string; lugar: string; empresaNombre: string;
  asunto: string; participantes: string; desarrollo: string; acuerdos: string;
  asuntosPendientes: string; observaciones: string;
};
...
export function ActaMinutaForm({ documentId, initialData }: { documentId?: string; initialData?: ActaMinutaInitialData }) {
  ...
  return <form className="minutes-form" onSubmit={save}>
    ...
    <label>Título<input name="titulo" required maxLength={180} defaultValue={initialData?.titulo || ""} placeholder="Ej.: Reunión con delegados de la empresa"/></label>
    <div className="minutes-grid">
      <label>Lugar<input name="lugar" maxLength={180} defaultValue={initialData?.lugar || ""}/></label>
      <label>Empresa o establecimiento<input name="empresa_nombre" maxLength={220} defaultValue={initialData?.empresaNombre || ""}/></label>
    </div>
    <SpeechTextarea label="Asunto" name="asunto" rows={2} initialValue={initialData?.asunto}/>
    ...
    <SpeechTextarea label="Desarrollo" name="desarrollo" rows={6} initialValue={initialData?.desarrollo}/>
    <SpeechTextarea label="Acuerdos alcanzados" name="acuerdos" rows={4} initialValue={initialData?.acuerdos}/>
    <SpeechTextarea label="Asuntos pendientes" name="asuntos_pendientes" rows={4} initialValue={initialData?.asuntosPendientes}/>
    <SpeechTextarea label="Observaciones" name="observaciones" rows={3} initialValue={initialData?.observaciones}/>
