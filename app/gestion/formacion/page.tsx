...
export default async function FormationPage() {
  ...
  const { data: courses, error } = await supabase.from("fs_cursos").select("id,titulo,subtitulo,descripcion_corta,nivel,modalidad,duracion_minutos,estado,version,imagen_portada").order("creado_en", { ascending:false });
  ...
  if (course) {
    ...
    if (modules.length) {
      ...
      if(lessons.length){const {data:progressRows}=await supabase.from("fs_progreso_lecciones").select("leccion_id").eq("usuario_id",user.id).eq("estado","completada").in("leccion_id",lessons.map(lesson=>lesson.id));completedLessonIds=new Set((progressRows??[]).map(row=>row.leccion_id));}
    }
  }
  ...
  return <main className="management">
    ...
    <section className="main-area formation-page"><header className="main-head"><div><p className="kicker">FORMACIÓN SINDICAL</p><h1>Capacitaciones</h1><p>Conocimiento para fortalecer la organización y la representación sindical.</p></div>{isAdmin&&<span className="review-badge">VISTA DE REVISIÓN</span>}</header>
      {error ? <div className="formation-empty"><h2>No pudimos consultar las capacitaciones</h2><p>Revisaremos la conexión con Formación Sindical.</p></div> : !course ? <div className="formation-empty"><h2>Próximamente</h2><p>Las primeras capacitaciones se encuentran en preparación.</p></div> : <>
        <article className="course-hero">{course.imagen_portada?<div className="course-cover"><Image src={course.imagen_portada} width={1600} height={1000} alt={`Portada de ${course.titulo}`}/></div>:<div className="course-mark">FS</div>}<div className="course-copy"><div className="course-meta"><span>{course.nivel||"Nivel inicial"}</span><span>{course.modalidad||"Autogestionada"}</span><span>Avanzá a tu propio ritmo</span></div><h2>{course.titulo}</h2>{course.subtitulo&&<h3>{course.subtitulo}</h3>}<p>{course.descripcion_corta}</p><div className="course-status"><strong>{course.estado}</strong><span>Versión {course.version}</span></div></div></article>
        <section className="course-content"><div className="section-heading"><div><p className="kicker">CONTENIDO DISPONIBLE</p><h2>Recorrido formativo</h2></div><span>{completedLessonIds.size} de {lessons.length} completadas</span></div>
          {modules.map(module=><article className="formation-module" key={module.id}><header><span className="module-number">{String(module.orden).padStart(2,"0")}</span><div><h3>{module.titulo}</h3><p>{module.descripcion}</p></div></header><div className="lesson-list">{lessons.filter(l=>l.modulo_id===module.id).map(lesson=><Link className={`lesson-row ${completedLessonIds.has(lesson.id)?"lesson-completed":""}`} href={`/gestion/formacion/lecciones/${lesson.id}`} key={lesson.id}><span>{completedLessonIds.has(lesson.id)?"✓":lesson.orden}</span><div><strong>{lesson.titulo}</strong><p>{lesson.proposito}</p></div><b>{completedLessonIds.has(lesson.id)?"Completada":"Abrir →"}</b></Link>)}</div></article>)}
