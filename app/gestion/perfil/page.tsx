...
export default async function ProfilePage({ searchParams }:{ searchParams:Promise<{estado?:string}> }) {
  ...
  const { data:profile } = await supabase.from("usuarios").select("nombre,apellido,dni,telefono,mail,rol,empresa,convenio,estado,activo").eq("id",user.id).maybeSingle();
