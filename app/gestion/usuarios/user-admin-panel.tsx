...
import { approveUser, deleteUserPermanently, rejectUser, updateInstitutionalUser } from "./actions";
...
export function UserAdminPanel({ users, companies, agreements, currentUserId }: { users:UserRow[]; companies:CatalogItem[]; agreements:CatalogItem[]; currentUserId:string }) {
    ...
    <div className="user-list">
      {visibleUsers.map((user)=><article className="user-card" key={user.id}>
        ...
        <div className="quick-actions">
          {user.estado==="Pendiente"&&<><form action={approveUser}><input type="hidden" name="id" value={user.id}/><button className="approve-user">Aprobar solicitud</button></form><form action={rejectUser}><input type="hidden" name="id" value={user.id}/><button className="reject-user">Rechazar</button></form></>}
          {user.id!==currentUserId&&<form action={deleteUserPermanently} onSubmit={(event)=>{if(!window.confirm(`¿Eliminar definitivamente la cuenta de ${[user.nombre,user.apellido].filter(Boolean).join(" ")}? Esta acción no se puede deshacer.`)) event.preventDefault();}}><input type="hidden" name="id" value={user.id}/><button className="delete-user">Eliminar definitivamente</button></form>}
        </div>
      </article>)}
