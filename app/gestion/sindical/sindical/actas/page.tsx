...
export default async function ActasPage() {
  ...
  const { data: documents } = await supabase.from("actas_minutas").select("id,tipo,titulo,fecha,estado,creado_por").order("fecha", { ascending: false }).limit(100);
  ...
  return <main className="management">
    ...
    <section className="main-area minutes-page"><Link className="library-back" href="/gestion/sindical">← Volver a Gestión Sindical</Link><header className="main-head"><div><p className="kicker">MEMORIA INSTITUCIONAL</p><h1>Actas y minutas</h1><p>Registrá reuniones, participantes, desarrollo y acuerdos en un formato claro y simple.</p></div></header>
      <details className="new-minutes" open><summary>Nueva acta o minuta</summary><ActaMinutaForm/></details>
      <section className="minutes-list"><h2>Documentos guardados</h2>{(documents || []).map((document) => <article key={document.id}><div><span>{String(document.tipo).toUpperCase()}</span><h3>{document.titulo}</h3><p>Registro institucional de reunión</p><Link className="open-minute" href={`/gestion/sindical/actas/${document.id}`}>Abrir documento →</Link></div><dl><div><dt>Fecha</dt><dd>{new Intl.DateTimeFormat("es-AR", { dateStyle: "short", timeStyle: "short" }).format(new Date(document.fecha))}</dd></div><div><dt>Responsable</dt><dd>{creatorNames.get(document.creado_por) || "Usuario autorizado"}</dd></div><div><dt>Estado</dt><dd>{document.estado}</dd></div></dl></article>)}{!documents?.length && <p className="minutes-empty">Todavía no hay actas ni minutas guardadas.</p>}</section>
    </section>
    ...
    <style>{`.open-minute{display:inline-block;margin-top:13px;color:#0b5264;font-size:14px;font-weight:900;text-decoration:none}:root[data-theme="dark"] .open-minute{color:#9ed9e5}`}</style>
  </main>;
}
