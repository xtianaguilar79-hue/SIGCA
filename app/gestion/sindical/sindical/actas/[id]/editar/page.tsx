...
export default async function EditarActaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const initialData: ActaMinutaInitialData = {
    tipo: value(document.tipo), titulo: value(document.titulo), fecha: localDateTime(document.fecha),
    participantes: value(document.participantes), desarrollo: value(document.desarrollo), acuerdos: value(document.acuerdos),
  };
